...
const util = new fruskac.Util();
const request = new fruskac.Request();
let event;
let i18n;
let storage;
let gmap;
...
fruskac.init = function () {
  ...
  const mapTypeIds = Object.values(google.maps.MapTypeId);
  mapTypeIds.push('osm');

  const mapConfig = {
    center: new google.maps.LatLng(
      fruskac.config.map.center.latitude,
      fruskac.config.map.center.longitude,
    ),
  ...
  const latLngZoom = request.get(PARAMETER_COORDINATES);
  if (latLngZoom) {
  ...
  const cyclosmBuckets = ['a', 'b', 'c'];
  gmap.mapTypes.set('osm', new google.maps.ImageMapType({
    getTileUrl(coord, zoom) {
      // See above example if you need smooth wrapping at 180th meridian
      return `https://${cyclosmBuckets[Math.round(coord.x + coord.y) % 3]}.tile-cyclosm.openstreetmap.fr/cyclosm/${zoom}/${coord.x}/${coord.y}.png`;
    },
  ...
  loader.load(fruskac.config.data).then(() => {
    if (track) {
      // load external track
      loader.append(track, TYPE_TRACK).then((object) => {
